...
```{r}
# Organize 2 treatments into same figure
biomass_df <- read.csv(paste0("/data/output/pecan_runs/temp_comp_results/comparison_diff_TotLivBiom.csv")) %>%
...
all_df <- rbind.data.frame(biomass_df, trans_df) %>%
  mutate(Trait = case_when(trait == "biomass" ~ "Delta~Biomass~(kg~m^-2)",
         ...
         Diff = case_when(diff == "rn_hn" ~ "cn - hn"))
...
vdecomp.b <- ggplot(data = vd1) +
  ...
  coord_flip() +
  geom_hline(aes(yintercept = 0), size = 0.1)  +
  scale_y_continuous(expression(paste("Var. explained (kg ", m^-2, ")")), 
...
vdecomp.t <- ggplot(data = vd2) +
  ...
  coord_flip() +
  geom_hline(aes(yintercept = 0), size = 0.1)  +
  scale_y_continuous(expression(paste("Var. explained (kg ", m^-2, " ", d^-1, ")")), 
                     breaks = pretty(vd$sd_convert, n = 3)) +
  theme_classic(base_size = 10) +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y=element_blank(), 
