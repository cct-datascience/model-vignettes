...
Two sets of figures comparing light and CO2 responses of Setaria under high night and regular night temperatures. 
...
aci_sum <- aci[, .(Ci.m = mean(Ci,  na.rm = T),
                   Ci.se = SE(Ci),
                   A.m = mean(A, na.rm = T),
                   A.se = SE(A),
...
aq <- as.data.table(aq)
aq_sum <- aq[, .(Qin.m = mean(Qin,  na.rm = T),
                 Qin.se = SE(Qin),
                 A.m = mean(A, na.rm = T),
                 A.se = SE(A),
...
for(i in 1:length(npq_list)) {
  ...
  npq_list[[i]] <- npq_list[[i]] %>%
    mutate(Fs_c = Fs - Fo, # Fs steady state fluorescence,  Fo in dark-adapted leaves
           Fm._c = Fm. - Fo, # Fm' (Fm.) maximal fluorescence from light-adapted leaves
           Fo._c = Fo. - Fo, # Minimum fluorescence intensity in the light-acclimated state
...
# Summarize
npq_sum <- npq[roundQin !=0 , .(A.m = mean(A, na.rm = T), A.se = SE(A),
                   Qin.m = mean(Qin,  na.rm = T), Qin.se = SE(Qin),
                   NPQ.m = mean(NPQ_c, na.rm = T), NPQ.se = SE(NPQ_c),
                   ETR.m = mean(ETR_c, na.rm = T), ETR.se = SE(ETR_c),
                   PhiPS2.m = mean(PhiPS2_c, na.rm = T), PhiPS2.se = SE(PhiPS2_c),
                   PhiPS2max.m = mean(PhiPS2max, na.rm = T), PhiPS2max.se = SE(PhiPS2max),
                   qP.m =  mean(qP, na.rm = T), qP.se = SE(qP),
                   qN.m = mean(qN, na.rm = T), qN.se = SE(qN)),  by = .(trt, roundQin)]
