...
### Examples Predicting from model
...
npp_pred <- predict(model_fit, newx, type = "numeric")
```

set up grid by adding environmental data for new sites, and keeping same param values

```{r}
splits <- function(x, n = 4){
  xx <- range(x)
  pts <- seq(min(x), max(x), length.out = n)
}
d <- data_train %>% 
  select(mean_temp, mean_vpd, mean_precip, mean_srad, mean_swe) %>% 
  summarise(mean_temp   = splits(mean_temp),
            mean_vpd = splits(mean_vpd),
            mean_precip = splits(mean_precip),
            mean_srad = splits(mean_srad),
            mean_swe = splits(mean_swe))

met_preds <- expand.grid(df = d)

newpreds <- data.frame(example_wt_data, met_preds)

npp_pred <- predict(model_fit, 
                    newpreds, 
                    type = "numeric")
#npp_pred_ci <- predict(model_fit, newpreds, type = "pred_int")

sa <- sa.samples$SetariaWT
newpreds_lowtemp <- newpreds %>% 
  mutate(stomatal_slope = sa$stomatal_slope[1],
         cuticular_cond = sa$cuticular_cond[1])

newpreds_antho <- newpreds %>% 
  mutate(quantum_efficiency = sa$quantum_efficiency[1])

newpreds_short <- newpreds %>% 
  mutate(fineroot2leaf = sa$fineroot2leaf[1])
...
d <- npp_pred - npp_pred_lowtemp

df <- data.frame(npp = npp_pred,
                 npp_antho = npp_pred_antho,
                 npp_short = npp_pred_short,
                 npp_lowt  = npp_pred_lowtemp,
                 d = d, 
                 mat = newpreds$mean_temp, 
                 map = newpreds$mean_precip*365)

df_long <- df %>% 
  pivot_longer(cols = starts_with('npp'))

ggplot(data = df_long, aes(x = mat, y = map, color = value)) +
  geom_point() +
  facet_wrap(~name)

ggplot(data = df, aes(x = mat, y = map, color = npp)) +
  geom_point()
...
```{r}
mets_mean_summer %>% summarise(range)
```
...
#                       breaks = c(0.3, 1, 1.2), 
#                       limits = c(0.3, 1.2)) +
  coord_cartesian(xlim = c(-113, -75), ylim = c(30, 45)) +
...
dp <- ggplot() +
  geom_polygon(data = NA_background, 
               aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = dpredslong, aes(x = lon, y = lat, fill = value), 
              alpha = 0.9) +
  # scale_fill_gradient2(low = "maroon", mid = "lightblue", high = "darkblue", 
  #                      midpoint = 0, 
  #                      breaks = c(-0.1, 0, 0.1), 
