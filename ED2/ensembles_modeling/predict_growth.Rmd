...
ggplot(ens_params_env, aes(x = mean_annual_temp, y = npp)) +
  geom_point() + 
  facet_grid(vars(site))
ggplot(ens_params_env, aes(x = mean_annual_precip, y = npp)) +
  geom_point() + 
  facet_grid(vars(site))
```
...
simplest approach
```{r}
#rf_fit <- fit_xy(rf_model, x = data_train %>% select(-c(ensemble, npp, site)), y = data_train$npp)
#have to use test data to get metrics
```
