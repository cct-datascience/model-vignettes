...
mandifore <- 
  mandifore_raw %>% 
  select(-duration, -created_at, -file_path) %>% 
...
new_sites <- full_join(met, cohort, by = c("site_id", "sitename", "lon", "lat")) %>% 
  ungroup()
new_sites
```
...
settings$run$inputs$met <- 
  file.path("/groups/dlebauer/data/sites/mandifore", s$met_filename)

files <- list.files(file.path("../../../../../data/input/", s$cohort_filename))

#TODO: fix these
settings$run$inputs$pss <- file.path("/groups/dlebauer/data/sites/mandifore", files[str_detect(files, "\\.pss$")])
settings$run$inputs$css <- file.path("/groups/dlebauer/data/sites/mandifore", files[str_detect(files, "\\.css$")])
settings$run$inputs$site <- file.path("/groups/dlebauer/data/sites/mandifore", files[str_detect(files, "\\.site$")])
```
...
Will this run as is, or do I need to specify PFTs extracted from cohort files??

TODO:
- Move MET files and cohort files to HPC
- Make sure the dir they are moved to is mounted in the "ED2 binary" .sh script on the HPC
