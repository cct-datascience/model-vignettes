...
settings <- PEcAn.settings::read.settings("ED2/transect_runs/SR/run/pecan.xml") 
...
# DATE = 2022-07-08-16-42-22
#rsync '-a' '-q' '--delete' '/data/tests/ed2_transect_SR/run' 'kristinariemer@login.ocelote.hpc.arizona.edu:/groups/dlebauer/ed2_results/pecan_remote/DATE'

# Start ecosystem model runs
PEcAn.remote::runModule.start.model.runs(settings, stop.on.error = FALSE)

# Manual rsync of out folder back to Welsch, replacing DATE
#rsync '-az' '-q' 'kristinariemer@login.ocelote.hpc.arizona.edu:/groups/dlebauer/ed2_results/pecan_remote/DATE/out' '/data/tests/ed2_transect_SR'

# Do results post-processing
for(folder in list.dirs("/data/tests/ed2_transect_SR/out", recursive = FALSE)){
  print(folder)
