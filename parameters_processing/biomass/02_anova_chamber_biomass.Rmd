...
ggplot(expdf, aes(x = age, y = abg_biomass_DW_mg, color = chamber, shape = tier)) +
  geom_point() +
  facet_grid(rows = vars(temp),
             cols = vars(light))

ggplot(expdf, aes(x = age, y = abg_biomass_DW_mg)) +
  geom_point() +
  facet_grid(rows = vars(temp),
             cols = vars(light))
...
ggplot(subset(expdf, temp == "31/31")) +
  geom_point(aes(x = as.factor(light), y = abg_biomass_DW_mg, col = chamber))
...
ggplot(subset(expdf, light == "430")) +
  geom_point(aes(x = temp, y = abg_biomass_DW_mg, col = chamber, shape = tier))

foo <- subset(expdf, light == "430" & temp == "31/31")
ggplot(foo, aes(x = chamber, y = abg_biomass_DW_mg, col = as.factor(exp_number))) +
  geom_point()

foo <- subset(expdf, light == "250" & temp == "31/31")
ggplot(foo, aes(x = chamber, y = abg_biomass_DW_mg, col = as.factor(exp_number))) +
  geom_point()
```
