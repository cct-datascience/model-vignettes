...
Test for Rd < 0
```{r}
which(masterDF$mean< 0 & masterDF$trait == "Rd")
ind <- which(ids %in% masterDF$ID[which(masterDF$mean< 0 & masterDF$trait == "Rd")])

for(i in ind){
  sub <- subset(datlist[[i]], type == "AQ")
  rd <- subset(masterDF, trait == "Rd" & ID == ids[i])
  
  sumDF <- coda.fast(out[[i]][[1]]$params)
  sumDF$trait2 <- row.names(sumDF)
  sumDF <- sumDF[c(grep("r0", fixed = T, sumDF$trait2)),]
  ggplot()+
    geom_point(data = sub, aes(x=Qin, y = A))+
    geom_hline(data = rd, aes(yintercept = -1*mean))+
    geom_hline(yintercept = -1*sumDF$mean, col = "red")+
    facet_wrap(~rep)
}
