...
for(i in 1:length(ids)){
  ...
  for(j in 3){# For types = Aci and AQ, as is
    sumDF <- coda.fast(out[[i]][[j]]$predict)[
      grep("pA", row.names(coda.fast(out[[i]][[j]]$predict))), c(1, 3:5)
    ]
    preds2 <- sumDF
...
for(i in 1:length(ids)){
  for(j in 1:length(types)){
    ...
    sumDF <- sumDF[c(grep("V[", fixed = T, sumDF$trait),
                     ...
                     grep("K[", fixed = T, sumDF$trait),
                     grep("r0", fixed = T, sumDF$trait)),]
    sumDF$ID <- rep(ids[i], nrow(sumDF))
    sumDF$type <- rep(types[j], nrow(sumDF))
    sumDF$trait <- c(rep("vmax", 3), rep("AQY", 3),"Rd")
    sumDF$rep <- c(rep(c("plant_1", "plant_2", "plant_3"), 2), "all")
...
plot(tapply(add$Value, add$ID, mean)[1:10], master$mean[master$type == "both" & master$trait == "Rd"])
...
fig1 <- ggplot(sub)+
  geom_point(aes(x = type, y = mean, color = rep), position = position_dodge(width = 0.5))+
...
sub2 <- subset(master, ID == ids[i] & type == "both")
fig2 <- ggplot(sub2)+
