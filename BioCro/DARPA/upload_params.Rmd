...
Read in previous phys_parameters.csv; will need for latest data. 
```{r}
...
Split dataframe, first by new data since last  upload, then by parameter.  
```{r}
...
#subset by date
sub <- subset(upload_parameters, local_datetime > max(phys$local_datetime))
write.csv(sub, file = paste0(newdir, "/upload_parameters.csv"), row.names = F)

#split dataframe for each parameter type and save each dataframe as a csv 
params <- c("Vcmax", "leaf_respiration_rate_m2", "quantum_efficiency", "stomatal_slope.BB", "cuticular_cond")
for(p in params){
  trait_df <- upload_parameters %>%
    filter(variable == p) %>%
    spread(variable, mean)
  
  # If Vcmax, rename columns n and SE 
  if(p == "Vcmax") {
    trait_df2 <- trait_df %>% dplyr::rename(Sample.size = n, Std.err = SE)} else {
      trait_df2 <- trait_df}
...
```{bash}
cd /home/jessicaguo/model-vignettes/BioCro/DARPA/upload_params_data

curl -X POST --data-binary @20200905/phys_parameters_Vcmax.csv http://welsch.cyverse.org:8000/bety/api/v1/traits.csv?key=uaZWRQT44fQVzDbxOET03EZJGXiEX9yUDEDiDwe4 -H "Content-Type: text/csv"

curl -X POST --data-binary @20200905/phys_parameters_leaf_respiration_rate_m2.csv http://welsch.cyverse.org:8000/bety/api/v1/traits.csv?key=uaZWRQT44fQVzDbxOET03EZJGXiEX9yUDEDiDwe4 -H "Content-Type: text/csv"

curl -X POST --data-binary @20200905/phys_parameters_quantum_efficiency.csv http://welsch.cyverse.org:8000/bety/api/v1/traits.csv?key=uaZWRQT44fQVzDbxOET03EZJGXiEX9yUDEDiDwe4 -H "Content-Type: text/csv"

curl -X POST --data-binary @20200905/phys_parameters_stomatal_slope.BB.csv http://welsch.cyverse.org:8000/bety/api/v1/traits.csv?key=uaZWRQT44fQVzDbxOET03EZJGXiEX9yUDEDiDwe4 -H "Content-Type: text/csv"

curl -X POST --data-binary @20200905/phys_parameters_cuticular_cond.csv http://welsch.cyverse.org:8000/bety/api/v1/traits.csv?key=uaZWRQT44fQVzDbxOET03EZJGXiEX9yUDEDiDwe4 -H "Content-Type: text/csv"
```
