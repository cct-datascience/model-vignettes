...
upload_parameters_ch <- all_parameters %>% 
  mutate(notes = paste("row", row_number(), "in https://docs.google.com/spreadsheets/d/1doQI0T7vav7LmNdMEoZJDx9iY_Imqa229CLRbRvurFc/#gid=967233489")) %>% 
  filter(genotype == "ME034", 
         TR %in% c("low night temp low light", 
                   "high night temp low light", 
                   "low night temp high light"), 
         trait %in% c("vmax", "Rd", "g1M", "g1BB", "g0BB")) %>% 
  ...
  select(-File, -genotype, -TR, -rep, -method, -SD, -TR.1) %>% 
  spread(trait, Value) %>% 
...
Split into a separate .csv file for each parameter type.
```{r}
vars<-colnames(upload_parameters_ch)[5:9]
for(i in vars){
  removecols <- setdiff(vars, i)
  new <- subset(upload_parameters_ch, select= setdiff(colnames(upload_parameters_ch), removecols))
  new2 <- new[complete.cases(new),]
  #remove leafT for stomatal_slope.g1
  new3 <- if(i == "stomatal_slope.g1") {subset(new2, select = -leafT)} else {new2}
  write.csv(new3, paste0("upload_params_data/phys_parameters_ch_", i, ".csv"), row.names = FALSE)
}
...
## Physiological parameter cuticular conductance

Has most of the same columns as the first set of parameters: `notes`, `local_datetime`, `treatment`, with `cuticular_cond` for the values. 

```{r}
upload_cc <- all_parameters %>% 
  mutate(notes = paste("row", row_number(), "in https://docs.google.com/spreadsheets/d/1doQI0T7vav7LmNdMEoZJDx9iY_Imqa229CLRbRvurFc/#gid=967233489")) %>% 
  filter(genotype == "ME034", 
         trait == "g0BB", 
         TR %in% c("low night temp low light", 
  ...
  mutate(date = ymd(paste0(str_sub(File, -4, -1), 
         ...
         treatment = case_when(TR == "low night temp low light" ~ "regular night temperature", 
                               TR == "high night temp low light" ~ "high night temperature", 
                               TR == "low night temp high light" ~ "high light"), 
         Value = ud.convert(Value, "mol/m2/s", "umol/m2/s"), 
         SE = ud.convert(SE, "mol/m2/s", "umol/m2/s")) %>% 
  select(-File, -genotype, -TR, -rep, -method, -New_Value, -SE.1, -SD) %>% 
  spread(trait, Value) %>%
  rename(local_datetime = date, 
         cuticular_cond = g0BB) %>% 
  mutate(leafT = 25, 
...
Save cuticular conductance values as a .csv. 
```{r}
write.csv(upload_cc, file = "upload_params_data/cut_cond.csv", row.names = FALSE)
```

## Greenhouse physiological parameters Vmax, Rd, and stomatal slope

Same as physiological parameters as above, but for greenhouse experiments. 
...
upload_parameters_gr <- all_parameters %>% 
  ...
  mutate(Date = ymd(paste0(str_sub(File, -4, -1), 
                           "-", match(str_sub(File, 1, 3), month.abb), 
                           "-", str_sub(File, -6, -5))), 
         treatment = case_when(TR == "greenhouse" ~ "greenhouse")) %>% 
  select(-File, -genotype, -TR, -rep, -method, -SD, -TR.1) %>% 
  spread(trait, Value) %>% 
  rename(local_datetime = Date, 
         Vcmax = vmax, 
...
write.csv(upload_parameters_gr, "upload_params_data/phys_parameters_gr.csv", row.names = FALSE)
```

Split into a separate .csv file for each parameter type.
```{r}
vars<-colnames(upload_parameters_gr)[5:9]
for(i in vars){
  removecols <- setdiff(vars, i)
  new <- subset(upload_parameters_gr, select= setdiff(colnames(upload_parameters_gr), removecols))
  new2 <- new[complete.cases(new),]
  #remove leafT for stomatal_slope.g1
  new3 <- if(i == "stomatal_slope.g1") {subset(new2, select = -leafT)} else {new2}
  write.csv(new3, paste0("upload_params_data/phys_parameters_gr_", i, ".csv"), row.names = FALSE)
}
