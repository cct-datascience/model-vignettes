...
control_biomass <- read_excel(data_path, sheets_names[7]) %>% 
...
convert_biomass <- function(x) ud.convert(x, "Mg/ha", "kg/m2")
daily_biomass <- data.frame(timescale, t(ensemble.ts[["TotLivBiom"]])) %>% 
  ...
  summarise(mean = sum(mean), 
            ...
            lcl = sum(lcl), 
            ucl = sum(ucl)) %>% 
  mutate_at(vars(mean:ucl), convert_biomass)
...
ggplot(data = daily_biomass) + 
  ...
  #geom_point(data = control_biomass, aes(x = days_grown, y = total_biomass_kgm2)) +
  xlab("Day of Year") + 
...
highnight_biomass <- read_excel(data_path, sheets_names[11]) %>% 
      rename(temperature...C..day.night = 6, 
             light.intensity = 7, 
             sowing.date = 8, 
...
# Plot measured biomass against biomass estimates
ggplot(data = daily_biomass) + 
  geom_line(aes(day, y = mean, color = "navyblue")) +
  geom_ribbon(aes(day, ymin = mean - 1 * sd, ymax = mean + 1 * sd, 
                  fill = "navyblue"), alpha = 0.1) +
  geom_ribbon(aes(day, ymin = mean - 2 * sd, ymax = mean + 2 * sd, 
                  fill = "blue"), alpha = 0.1) +
  geom_point(data = highnight_biomass, aes(x = days_grown, y = total_biomass_kgm2, 
                                           color = "black")) +
  scale_color_identity(name = "", 
                       breaks = c("navyblue", "black"), 
                       labels = c("Modeled", "Measured"), 
                       guide = "legend") +
  scale_fill_identity(name = "", 
                    breaks = c("navyblue", "blue"), 
                    labels = c("Mean +/- 1 standard deviation", "Mean +/- 2 standard deviations"), 
                    guide = "legend") +
  xlim(x = c(0, 50)) +
...
hourly_transp <- data.frame(timescale, t(ensemble.ts[["TVeg"]])) %>% 
  gather(ensemble, transpiration, X1:X10) %>% 
  group_by(day, hour) %>% 
  summarise(mean = mean(transpiration, na.rm = TRUE), 
            sd = sd(transpiration, na.rm = TRUE)) %>%
  mutate_at(vars(mean:sd), convert_transp) %>% 
  mutate(date = as.Date(day, origin = "2019-04-11"), 
...
daily_transp <- data.frame(timescale, t(ensemble.ts[["TVeg"]])) %>% 
  ...
  summarise(mean = mean(transpiration, na.rm = TRUE), 
            ...
            sd = sd(transpiration, na.rm = TRUE), 
            lcl = quantile(transpiration, probs = c(0.025), na.rm = TRUE), 
            ucl = quantile(transpiration, probs = c(0.975), na.rm = TRUE)) %>% 
  group_by(day) %>% 
  summarise(mean = sum(mean), 
            median = sum(median), 
            sd = sqrt(sum(sd^2)), 
            lcl = sum(lcl), 
            ucl = sum(ucl)) %>%
  mutate_at(vars(mean:ucl), convert_transp)
#write.csv(daily_transpiration, "temp_exps_inputs3/transpiration_ests3.csv")
rm(ensemble.ts)

# Plot transpiration estimates
ggplot(data = hourly_transp) + 
  geom_line(aes(x = date_hour, y = mean, color = "darkred"))+
  geom_ribbon(aes(x = date_hour, ymin = mean - 1 * sd, ymax = mean + 1 * sd, 
                  fill = "darkred"), alpha = 0.1) +
  geom_ribbon(aes(x = date_hour, ymin = mean - 2 * sd, ymax = mean + 2 * sd, 
                  fill = "red"), alpha = 0.1) +
  scale_color_identity(name = "", 
                       breaks = "darkred", 
                       labels = "Modeled", 
                       guide = "legend") +
  scale_fill_identity(name = "", 
                    breaks = c("darkred", "red"), 
                    labels = c("Mean +/- 1 standard deviation", "Mean +/- 2 standard deviations"), 
                    guide = "legend") +
  #xlim(x = c("2019-05-06 00:00:00", "2019-05-13 00:00:00")) +
  scale_x_datetime(limits = lubridate::ymd_hms(c("2019-05-06 00:00:00", "2019-05-10 00:00:00"))) +
  xlab("Date") + 
  ylab("Transpiration (kg/m2/h)") +
  theme_classic() +
  theme(legend.position = "bottom")

ggplot(data = daily_transp) + 
  geom_line(aes(day, y = mean, color = "darkred")) +
  geom_ribbon(aes(day, ymin = mean - 1 * sd, ymax = mean + 1 * sd, 
                  fill = "darkred"), alpha = 0.1) +
  geom_ribbon(aes(day, ymin = mean - 2 * sd, ymax = mean + 2 * sd, 
                  fill = "red"), alpha = 0.1) +
...
Sensitivity analysis and variance decomposition plots for high night temperature results. 
```{r}
load("temp_exps_results/temp_exps_results1/sensitivity.results.NOENSEMBLEID.TotLivBiom.2019.2019.Rdata")
...
ggplot(data = biomass_ests) +
  geom_line(aes(day, mean, color = run)) +
  geom_point(data = biomass_meas, aes(x = days_grown, y = total_biomass_kgm2, color = txt)) +
