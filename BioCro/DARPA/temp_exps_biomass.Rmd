...
highnight_biomass <- read_excel(data_path, sheets_names[11]) %>% 
  ...
  mutate(days_grown = as.integer(as.Date(as.character(biomass.harvested), format = "%Y-%m-%d") - 
                                   as.Date(as.numeric(temperature_treatment_started), origin = "1899-12-30")),
         total_biomass_mg = panicles.DW..mg. + stemDW.mg. + leaf.DW.mg. + roots.DW..mg., 
         total_biomass_Mgha = ud.convert(total_biomass_mg, "mg", "Mg") / area_ha, 
         total_biomass_gcm2 = ud.convert(total_biomass_Mgha, "Mg/ha", "g/cm2")) %>% 
  filter(!is.na(total_biomass_Mgha))
...
convert_biomass <- function(x) ud.convert(x, "Mg/ha", "g/cm2")
daily_biomass <- data.frame(timescale, t(ensemble.ts[["TotLivBiom"]])) %>% 
  gather(ensemble, biomass, X1:X10) %>% 
  group_by(day, hour) %>% 
  summarise(mean = mean(biomass, na.rm = TRUE), 
...
ggplot(data = daily_biomass) + 
  ...
  geom_ribbon(aes(day, ymin = mean - sd_scale * sd, ymax = mean + sd_scale * sd, 
                  fill = "navyblue"), alpha = 0.1) +
  geom_ribbon(aes(day, ymin = lcl, ymax = ucl, fill = "blue"), alpha = 0.1) +
  geom_point(data = highnight_biomass, aes(x = days_grown, y = total_biomass_gcm2, 
                                           color = "black")) +
  scale_color_identity(name = "", 
                       breaks = c("navyblue", "black"), 
                       labels = c("Mean estimates", "Measured biomass"), 
                       guide = "legend") +
  scale_fill_identity(name = "", 
                    breaks = c("navyblue", "blue"), 
                    labels = c("first CI", "second CI"), 
                    guide = "legend") +
  xlim(x = c(0, 50)) +
  xlab("Day of Year") + 
  ylab("Total Biomass (g/cm2)") +
  theme_classic() +
