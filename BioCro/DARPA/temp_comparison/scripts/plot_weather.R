...
rn <- read.csv("BioCro/DARPA/temp_comparison/inputs/weather.rn.2019.csv") %>%
  mutate(dt = as.POSIXct((doy-1)*24*60*60 + hour*60*60, origin = "2019-01-01"),
         trt = "rn")

hn <- read.csv("BioCro/DARPA/temp_comparison/inputs/weather.hn.2019.csv") %>%
  mutate(dt = as.POSIXct((doy-1)*24*60*60 + hour*60*60, origin = "2019-01-01"),
         trt = "hn")


all_wide <- rbind(rn, hn) %>%
  relocate(trt, dt)
all_long <- all_wide %>%
...
fig_all_comb <- ggplot(all_wide, aes(x = dt)) +
  ...
  geom_bar(aes(y = precip*10, color = "precip"), stat = "identity", show.legend = FALSE) +
