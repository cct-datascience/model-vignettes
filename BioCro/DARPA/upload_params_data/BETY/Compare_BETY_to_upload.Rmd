...
Data from BETY is selecte with the following SQL query: select * from traits where treatment_id in (9000000015, 9000000016, 9000000017, 9000000018, 9000000019, 9000000020)
The results are saved as csv in the upload_params_data/BETY folder as the most recent 'update_at' date. 
...
library(dplyr)
library(arsenal)
#import newest BETY file
filename <- max(as.numeric(substr(list.files(pattern = "csv"), 1,8)))
in_bety <- read.csv(paste0(filename, ".csv"), header = T)
#import full upload_params_data data file
in_upload <- read.csv("../phys_parameters.csv",  header = T)
```

Convert the treatment and variable ID columns to match
```{r}
bety <- in_bety %>% 
  mutate(date = as.POSIXct(as.character(date), format = "%d/%m/%Y %H:%M:%S"),
         treatment = case_when(treatment_id == "9000000015" ~ "regular night temperature",
                               ...
                               treatment_id == "9000000017" ~ "high night",
         ...
         variable = case_when(variable_id == "7" ~ "leaf_respiration_rate",
...
If no differences, should return 0 variables and 0 observations not shared. 