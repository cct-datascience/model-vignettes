...
estimate_biomass <- function(genotype){
  ...
  results_path <- paste0("regional_runs_inputs/results/stlew_", genotype, ".csv")
...
for(genotype in genotypes){
  biomass_path <- paste0("regional_runs_inputs/results/stlew_", genotype, ".csv")
...
rr_results <- rr_results %>% 
  mutate(date = as.Date(doy, "2009-12-31"), 
         ...
         genotype_formal = case_when(
           ...
           genotype == "ss" ~ "Decreased stomata", 
           genotype == "ca" ~ "Changed carbon allocation" 
         ), 
         genotype_formal = factor(genotype_formal, levels = c("Wild type", "Decreased stomata", "Changed carbon allocation")))
```
...
biomass_animation <- ggplot() +
  ...
  scale_fill_gradient2(low = "yellow", mid = "yellowgreen", high = "darkgreen", 
                       ...
                       limits = c(0, 2)) +
  coord_cartesian(xlim = c(-113, -80), ylim = c(34, 42)) +
...
animate(biomass_animation, fps = 100, height = 600, width = 400)
anim_save("stlew_biomass.gif", animation = biomass_animation, 
          path = "regional_runs_inputs/results/", fps = 100, height = 600, width = 400)
```
...
```{r}
bio_comp_1 <- ggplot() +
  geom_line(data = filter(rr, genotype_formal %in% c("Wild type", "Decreased stomata")), 
            aes(x = date, y = mean_total_biomass, group = stringr::str_wrap(genotype_formal, 12), 
                color = stringr::str_wrap(genotype_formal, 12))) +
  geom_ribbon(data = filter(rr, genotype_formal %in% c("Wild type", "Decreased stomata")), 
...
bio_comp_2 <- ggplot() +
  geom_line(data = filter(rr, genotype_formal %in% c("Wild type", "Changed carbon allocation")), 
            aes(x = date, y = mean_total_biomass, group = stringr::str_wrap(genotype_formal, 12), 
                color = stringr::str_wrap(genotype_formal, 12))) +
  geom_ribbon(data = filter(rr, genotype_formal %in% c("Wild type", "Changed carbon allocation")), 
              aes(x = date, ymin = mean_total_biomass - var_total_biomass, 
                  ymax = mean_total_biomass + var_total_biomass,
                  group = stringr::str_wrap(genotype_formal, 12), fill = stringr::str_wrap(genotype_formal, 12)), alpha = 0.2) +
  scale_x_datetime(labels = scales::date_format("%b")) +
...
animate(transpiration_animation, fps = 100, height = 600, width = 400)
...
trans_comp_1 <- ggplot() +
  geom_line(data = filter(rr, genotype_formal %in% c("Wild type", "Decreased stomata")), 
            aes(x = date, y = mean_transpiration, group = stringr::str_wrap(genotype_formal, 12), 
                color = stringr::str_wrap(genotype_formal, 12))) +
  geom_ribbon(data = filter(rr, genotype_formal %in% c("Wild type", "Decreased stomata")), 
