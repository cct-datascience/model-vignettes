...
rr <- regional_run_results %>% 
  select(date, lat, lon, latlon, total_biomass, transpiration)
rr_ss <- regional_run_results_ss %>% 
  select(date, latlon, total_biomass, transpiration) %>% 
  rename(total_biomass_ss = total_biomass, 
         transpiration_ss = transpiration)
...
rr_ss_mean <- rr_ss %>% 
  ...
  summarize(mean_total_biomass = mean(total_biomass_ss), 
            ...
            mean_transpiration = mean(transpiration_ss), 
            var_transpiration = sd(transpiration_ss)) %>% 
  mutate(genotype = "Modified stomata")
rr_comb_ss <- bind_rows(rr_mean, rr_ss_mean)

ggplot() +
  geom_line(data = rr_comb_ss, aes(x = date, y = mean_total_biomass, 
                                   group = genotype, color = genotype)) +
  geom_ribbon(data = rr_comb_ss, aes(x = date, ymin = mean_total_biomass - var_total_biomass, ymax = mean_total_biomass + var_total_biomass, 
                                     group = genotype, fill = genotype), alpha = 0.2) +
  scale_x_datetime(labels = scales::date_format("%b")) +
  labs(x = "Month", y = "Biomass (kg/m2)") +
  theme_classic()
```
...
```{r}
ggplot() +
  geom_line(data = rr_comb_ss, aes(x = date, y = mean_transpiration, 
                                   group = genotype, color = genotype)) +
  geom_ribbon(data = rr_comb_ss, aes(x = date, ymin = mean_transpiration - var_transpiration, ymax = mean_transpiration + var_transpiration, 
                                  group = genotype, fill = genotype), alpha = 0.2) +
  scale_x_datetime(labels = scales::date_format("%b")) +
  labs(x = "Month", y = "Transpiration (kg/m2/hr)") +
  theme_classic()
```
...
rr_ca <- regional_run_results_ca %>% 
  ...
  rename(total_biomass_ca = total_biomass, 
         transpiration_ca = transpiration)
...
rr_ca_mean <- rr_ca %>% 
  ...
  summarize(mean_total_biomass = mean(total_biomass_ca), 
            ...
            mean_transpiration = mean(transpiration_ca), 
            var_transpiration = sd(transpiration_ca)) %>% 
  mutate(genotype = "Changed carbon allocation")
rr_comb_ca <- bind_rows(rr_mean, rr_ca_mean)

ggplot() +
  geom_line(data = rr_comb_ca, aes(x = date, y = mean_total_biomass, 
                                   group = genotype, color = genotype)) +
  geom_ribbon(data = rr_comb_ca, aes(x = date, ymin = mean_total_biomass - var_total_biomass, ymax = mean_total_biomass + var_total_biomass, 
                                     group = genotype, fill = genotype), alpha = 0.2) +
  scale_x_datetime(labels = scales::date_format("%b")) +
  labs(x = "Month", y = "Biomass (kg/m2)") +
  theme_classic()
...
```{r}
ggplot() +
  geom_line(data = rr_comb_ca, aes(x = date, y = mean_transpiration, 
                                   group = genotype, color = genotype)) +
  geom_ribbon(data = rr_comb_ca, aes(x = date, ymin = mean_transpiration - var_transpiration, ymax = mean_transpiration + var_transpiration, 
                                  group = genotype, fill = genotype), alpha = 0.2) +
  scale_x_datetime(labels = scales::date_format("%b")) +
  labs(x = "Month", y = "Transpiration (kg/m2/hr)") +
  theme_classic()
