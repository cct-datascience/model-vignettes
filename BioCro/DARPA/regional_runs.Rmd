...
```{r}
config <- read.biocro.config("BioCro/DARPA/regional_runs_inputs/config.xml")
```
...
```{r}
metfile <- "BioCro/DARPA/regional_runs_inputs/chiruss.nc"
met_chiruss <- nc_open(metfile)
soil_nc <- nc_open("BioCro/DARPA/regional_runs_inputs/hwsd.nc")
```
...
dir.create("BioCro/DARPA/regional_runs_inputs/biocro_met_by_location/")
...
for(point in 1:nrow(latlon)){
  ...
  biocro_met_path <- paste0("BioCro/DARPA/regional_runs_inputs/biocro_met_by_location/biocromet-", 
...
```{r}
dir.create("BioCro/DARPA/regional_runs_inputs/results_by_location/")
biocro_results <- c()
for(point in 1:nrow(latlon)){
  biocro_met_path <- paste0("BioCro/DARPA/regional_runs_inputs/biocro_met_by_location/biocromet-", 
  ...
  write.csv(biocro_results_daily, paste0("BioCro/DARPA/regional_runs_inputs/results_by_location/daily-", 
  ...
  write.csv(biocro_results, "BioCro/DARPA/regional_runs_inputs/all_biocro_results.csv")
}
```
...
```{r}
biocro_results <- read.csv("BioCro/DARPA/regional_runs_inputs/chiruss_results/all_biocro_results.csv")
...
ecoregions <- readOGR(file.path("BioCro/DARPA/regional_runs_inputs", 
...
anim_save("biomass_animation_chiruss.gif", animation = biomass_animation, 
          path = "BioCro/DARPA/regional_runs_inputs/", fps = 100)
```
