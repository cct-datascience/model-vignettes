...
biomass_animation <- ggplot() +
  ...
  labs(x = "", y = "", fill = "Biomass (kg/m2)") +
  transition_manual(doy) +
  ggtitle('Day: {current_frame}') +
  coord_quickmap()

animate(biomass_animation, fps = 100, height = 200, width = 400)
anim_save("high_light_stlew.gif", animation = biomass_animation, 
          path = "regional_runs_inputs/results/", fps = 100, height = 200, width = 400)
```

## Plot estimated transpiration

The results for evapotranspiration for each location across the year of 2010 are visualized. 

```{r}
ggplot(regional_run_results, aes(x = date, y = transpiration, group = latlon, color = latlon)) +
  geom_line(alpha = 0.2) +
...
transpiration_animation <- ggplot() +
  geom_polygon(data = gif_background, aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = regional_run_results, aes(x = lon, y = lat, fill = transpiration)) +
  geom_point(data = city_coords, aes(x = lon, y = lat)) +
  ...
  theme(panel.background = element_rect(fill = "grey", colour = "grey"), 
        panel.grid.major = element_line(colour = "grey"),
        panel.grid.minor = element_line(colour = "grey")) +
  labs(x = "", y = "", fill = "Transpiration (kg/m2/hr)") +
  transition_manual(doy) +
  ggtitle('Day: {current_frame}') +
  coord_quickmap()

animate(transpiration_animation, fps = 100, height = 200, width = 400)
anim_save("high_light_stlew_trans.gif", animation = transpiration_animation, 
          path = "regional_runs_inputs/results/", fps = 100, height = 200, width = 400)
```
...
animate(biomass_animation_ss, fps = 100, height = 200, width = 400)
...
```{r}
ggplot(regional_run_results_ss, aes(x = date, y = transpiration, group = latlon, color = latlon)) +
  geom_line(alpha = 0.2) +
  labs(y = "Transpiration (kg/m2/hr)") +
  theme(legend.position = "none")
```
...
transpiration_animation <- ggplot() +
  geom_polygon(data = gif_background, aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = regional_run_results_ss, aes(x = lon, y = lat, fill = transpiration)) +
  geom_point(data = city_coords, aes(x = lon, y = lat)) +
  scale_fill_gradient2(low = "lightblue", mid = "blue", high = "navyblue", 
                       midpoint = 7.5, 
                       breaks = seq(0, 20, 5), 
...
animate(transpiration_animation, fps = 100, height = 200, width = 400)
anim_save("high_light_ss_stlew_trans.gif", animation = transpiration_animation, 
          path = "regional_runs_inputs/results/", height = 200, width = 400)
```
...
rr <- regional_run_results %>% 
  select(date, lat, lon, latlon, total_biomass, CanopyTrans)
rr_ss <- regional_run_results_ss %>% 
  select(date, latlon, total_biomass, CanopyTrans) %>% 
  rename(total_biomass_ss = total_biomass, 
         CanopyTrans_ss = CanopyTrans)
comb <- left_join(rr, rr_ss, by = c("date", "latlon")) %>% 
  mutate(total_biomass_diff = total_biomass_ss - total_biomass)
```

Comparison of time series of mean and variation of total biomass by date. 
...
rr_mean <- rr %>% 
  ...
  summarize(mean_total_biomass = mean(total_biomass), 
            ...
            mean_transpiration = mean(CanopyTrans), 
            var_transpiration = sd(CanopyTrans))
rr_ss_mean <- rr_ss %>% 
  group_by(date) %>% 
  summarize(mean_total_biomass = mean(total_biomass_ss), 
            var_total_biomass = sd(total_biomass_ss),
            mean_transpiration = mean(CanopyTrans_ss), 
            var_transpiration = sd(CanopyTrans_ss))

ggplot() +
  geom_line(data = rr_mean, aes(x = date, y = mean_total_biomass)) +
  geom_ribbon(data = rr_mean, aes(x = date, ymin = mean_total_biomass - var_total_biomass, ymax = mean_total_biomass + var_total_biomass), alpha = 0.2) +
  geom_line(data = rr_ss_mean, aes(x = date, y = mean_total_biomass), color = "blue") +
  geom_ribbon(data = rr_ss_mean, aes(x = date, ymin = mean_total_biomass - var_total_biomass, ymax = mean_total_biomass + var_total_biomass), alpha = 0.2, fill = "blue") +
  theme(legend.position = "none")
```
...
Comparison of time series of mean and variation of transpiration by date. 

```{r}
ggplot() +
  geom_line(data = rr_mean, aes(x = date, y = mean_transpiration)) +
  geom_ribbon(data = rr_mean, aes(x = date, ymin = mean_transpiration - var_transpiration, ymax = mean_transpiration + var_transpiration), alpha = 0.2) +
  geom_line(data = rr_ss_mean, aes(x = date, y = mean_transpiration), color = "blue") +
  geom_ribbon(data = rr_ss_mean, aes(x = date, ymin = mean_transpiration - var_transpiration, ymax = mean_transpiration + var_transpiration), alpha = 0.2, fill = "blue") +
  theme(legend.position = "none")
```
...
This set of runs uses the same parameters as above for the high light treatment, except in `config.xml` the values of all `kStem` are halved, with the remaining half split evenly between the other plant parts by stage. 

```{r}
config_ca <- read.biocro.config("regional_runs_inputs/config_ca.xml")
```

## Run BioCro on each location

```{r}
dir.create("regional_runs_inputs/results/high_light_ca_stlew")

regional_run_results_ca <- c()
for(point in 1:nrow(latlon)){
  biocro_met_path <- paste0("regional_runs_inputs/biocro_met_stlew/stlew-", 
                            latlon$lat[point], "-", latlon$lon[point])
  regional_run_result_ca <- run.biocro(latlon$lat[point], latlon$lon[point],
                               metpath = biocro_met_path,
                               soil.nc = soil_nc,
                               config = config_ca)
  regional_run_result_ca_daily <- regional_run_result_ca$daily
  write.csv(regional_run_result_ca_daily, paste0("regional_runs_inputs/results/high_light_ca_stlew/daily-", 
                                         latlon$lat[point], "-", latlon$lon[point], ".csv"))
  regional_run_result_ca_daily$lat <- latlon$lat[point]
  regional_run_result_ca_daily$lon <- latlon$lon[point]
  regional_run_results_ca <- rbind(regional_run_results_ca, regional_run_result_ca_daily)
  print(point)
}

write.csv(regional_run_results_ca, 
          "regional_runs_inputs/results/high_light_ca_stlew_results.csv")
```

## Plot estimated biomass

```{r}
regional_run_results_ca <- read.csv("regional_runs_inputs/results/high_light_ca_stlew_results.csv")
regional_run_results_ca <- regional_run_results_ca %>% 
  mutate(date = as.Date(doy, "2009-12-31"), 
         latlon = paste0(lat, lon), 
         total_biomass = ud.convert(as.numeric(Stem + Leaf + Root + Rhizome + Grain),
                                    "Mg/ha", "kg/m2"), 
...
ggplot(regional_run_results_ca, aes(x = date, y = total_biomass, group = latlon, color = latlon)) +
  geom_line() +
  theme(legend.position = "none")
```

```{r}
summer_day_plot_ca <- ggplot() +
  geom_polygon(data = overall_map, aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = filter(regional_run_results_ca, doy == 200), aes(x = lon, y = lat, fill = total_biomass)) +
  coord_quickmap() +
  ggtitle("Day 200")

winter_day_plot_ca <- ggplot() +
  geom_polygon(data = overall_map, aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = filter(regional_run_results_ca, doy == 350), aes(x = lon, y = lat, fill = total_biomass)) +
  coord_quickmap() +
  ggtitle("Day 350")

cowplot::plot_grid(summer_day_plot_ca, winter_day_plot_ca, nrow = 1)
```
...
#   theme(legend.position = "none") +
#   geom_raster(data = filter(regional_run_results_ca, doy == 200), aes(x = lon, y = lat, fill = total_biomass)) +
#   coord_cartesian(xlim = c(mean(regional_run_results_ca$lon) - 20, mean(regional_run_results_ca$lon) + 20),
#                   ylim = c(min(regional_run_results_ca$lat) - 5, max(regional_run_results_ca$lat) + 5)) +
#   theme(legend.position = "none")
```

```{r}
biomass_animation_ca <- ggplot() +
  geom_polygon(data = gif_background, aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = regional_run_results_ca, aes(x = lon, y = lat, fill = total_biomass)) +
  geom_point(data = city_coords, aes(x = lon, y = lat)) +
  scale_fill_gradient2(low = "yellow", mid = "yellowgreen", high = "darkgreen", 
                       midpoint = 1,
                       breaks = seq(-10, 10, 0.4),
...
animate(biomass_animation_ca, fps = 100, height = 200, width = 400)
anim_save("high_light_ca_stlew.gif", animation = biomass_animation_ca, 
          path = "regional_runs_inputs/results/", fps = 100, height = 200, width = 400)
```
...
```{r}
ggplot(regional_run_results_ca, aes(x = date, y = transpiration, group = latlon, color = latlon)) +
  geom_line(alpha = 0.2) +
  labs(y = "Transpiration (kg/m2/hr)") +
  theme(legend.position = "none")
```
...
transpiration_animation <- ggplot() +
  geom_polygon(data = gif_background, aes(x = long, y = lat, group = group), 
               fill = "white", color = "black") +
  geom_raster(data = regional_run_results_ca, aes(x = lon, y = lat, fill = transpiration)) +
  geom_point(data = city_coords, aes(x = lon, y = lat)) +
  scale_fill_gradient2(low = "lightblue", mid = "blue", high = "navyblue", 
                       midpoint = 7.5, 
                       breaks = seq(0, 20, 5), 
...
animate(transpiration_animation, fps = 100, height = 200, width = 400)
anim_save("high_light_ca_stlew_trans.gif", animation = transpiration_animation, 
          path = "regional_runs_inputs/results/", fps = 100, height = 200, width = 400)
```
...
Combining data from high light and changed carbon allocation regional runs.

```{r}
rr <- regional_run_results %>% 
  select(date, lat, lon, latlon, total_biomass, CanopyTrans)
rr_ca <- regional_run_results_ca %>% 
  select(date, latlon, total_biomass, CanopyTrans) %>% 
  rename(total_biomass_ca = total_biomass, 
         CanopyTrans_ca = CanopyTrans)
comb <- left_join(rr, rr_ca, by = c("date", "latlon")) %>% 
  mutate(total_biomass_diff = total_biomass_ca - total_biomass)
```
...
rr_ca_mean <- rr_ca %>% 
  group_by(date) %>% 
  summarize(mean_total_biomass = mean(total_biomass_ca), 
            var_total_biomass = sd(total_biomass_ca),
            mean_transpiration = mean(CanopyTrans_ca), 
            var_transpiration = sd(CanopyTrans_ca))

ggplot() +
  geom_line(data = rr_mean, aes(x = date, y = mean_total_biomass)) +
  geom_ribbon(data = rr_mean, aes(x = date, ymin = mean_total_biomass - var_total_biomass, ymax = mean_total_biomass + var_total_biomass), alpha = 0.2) +
  geom_line(data = rr_ca_mean, aes(x = date, y = mean_total_biomass), color = "red") +
  geom_ribbon(data = rr_ca_mean, aes(x = date, ymin = mean_total_biomass - var_total_biomass, ymax = mean_total_biomass + var_total_biomass), alpha = 0.2, fill = "red") +
  theme(legend.position = "none")
```
...
ggplot() +
  geom_line(data = rr_mean, aes(x = date, y = mean_transpiration)) +
  geom_ribbon(data = rr_mean, aes(x = date, ymin = mean_transpiration - var_transpiration, ymax = mean_transpiration + var_transpiration), alpha = 0.2) +
  geom_line(data = rr_ca_mean, aes(x = date, y = mean_transpiration), color = "red") +
  geom_ribbon(data = rr_ca_mean, aes(x = date, ymin = mean_transpiration - var_transpiration, ymax = mean_transpiration + var_transpiration), alpha = 0.2, fill = "red") +
  theme(legend.position = "none")
```
