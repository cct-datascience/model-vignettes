...
OpBioGro_weather <- read.csv("BioCro/DARPA/biocro_opt_darpa_files/OpBioGro_weather.csv")
...
config <- PEcAn.BIOCRO::read.biocro.config(file.path(rundir, "BioCro/DARPA/biocro_opt_darpa_files/config.xml"))
...
OpBioGro_biomass <- read.csv("BioCro/DARPA/biocro_opt_darpa_files/OpBioGro_biomass.csv")
```
...
TODO:
  ...
  - additional param constraints: parameter values for each stage should
    sum to one (note: this MUST happen because `BioCro` will not use
    pheno params that do not equal one)
      ...
      - another option for this sum to one constraint is something like
        the following for each stage as the first part of `opfn`
...
optimizingParms <- c(rep(c(0.2, 0.3, 0.5), 6), 0)
thermaltimeParms <- c(config$pft$phenoParms$tp1, config$pft$phenoParms$tp2, 
                      config$pft$phenoParms$tp3, config$pft$phenoParms$tp4, 
                      config$pft$phenoParms$tp5, config$pft$phenoParms$tp6)
rhizomeParms <- rep(0, 6)

opfn <- function(optimizingParms, thermaltimeParms, rhizomeParms){ 

  # Attempts at getting sum of leaf, stem, and root parameters in each stage to equal one:
  
  # 1. doesn't work because the likelihood of them being equal to one are very low, so it always returns Inf values (see unique(opt_results$member$bestvalit))
  # if(sum(optimizingParms[1:3]) != 1){
  #     return(Inf)
  # } else {
  
  # 2. not sure where this would go to be effective? 
  # optimizingParms[1:3] <- sum(optimizingParms[1:3]/sum(optimizingParms[1:3]))
...
opfn(optimizingParms, thermaltimeParms, rhizomeParms)
```
...
opt_results <- DEoptim(fn = opfn, lower = rep(0, 19), upper = rep(1, 19), thermaltimeParms = thermaltimevals, rhizomeParms = rhizomevals, control = DEoptim.control(itermax = 1000))

# rerun prior line until opt_results$optim$bestval != Inf? 
...
results_test == opt_results$optim$bestval
```
