...
# as.POSIXct(paste(unlist(strsplit(tunits$value, " "))[3], unlist(strsplit(tunits$value, " "))[4])) + 3*60*60

FSDS <- ncvar_get(clm_met, "FSDS") * 4.6
time_FSDS <- data.table::data.table(data.frame(date = seq(as.POSIXct(strptime("2000-01-01 00:00:00", "%Y-%m-%d %H:%M:%S")), as.POSIXct(strptime("2000-12-30 21:00:00", "%Y-%m-%d %H:%M:%S")), 3*60*60), 
                        year = rep(2000, 2920), 
...
new.date <- time_FSDS[,list(hour = 0:(23 / output.dt) / output.dt),
                        by = c("year", "month", "day", "doy")]
new.date$date <- new.date[,list(date = lubridate::ymd_h(paste(year, month, day, hour)))]


f <- stats::splinefun(as.double(time_FSDS$date), (time_FSDS$FSDS / hrscale), method = "monoH.FC")
