...
This shows where the locations are in the two countries
```{r}
...
```{r}
#TODO: close up with ecoregions

background_map <- map_data("world") %>% 
  filter(region == "China" | region == "Russia") 
# %>% 
#   filter(long > 65 & long < 110) %>% 
#   filter(lat > 25 & lat < 75)
...
ecoregions_df <- plyr::join(ecoregions_points, ecoregions@data, by = "id")
 ...
 ecoregions_df2 <- ecoregions_df %>% 
   filter(lat > 20 & lat < 80,
          long > 50 & long < 150)

colfunc <- colorRampPalette(c("yellow", "darkgreen"))
ggplot() +
  geom_polygon(data = ecoregions_df2, aes(long, lat, group = group, color = ECO_NAME)) +
  geom_polygon(data = background_map, aes(x = long, y = lat, group = group), fill = NA, color = "black") +
  geom_raster(data = biocro_results, aes(x = lon, y = lat), fill = "red") +
  scale_fill_gradientn(colors = colfunc(10)) +
  #coord_cartesian(xlim = c(65, 110), ylim = c(25, 75)) +
  coord_quickmap() +
  theme(legend.position = "none")
```

The code below plots these values across the 18 locations on a map of the state of Illinois, and then runs through these plots for each day of the year as a .gif. 
```{r}
#TODO: close up gif
background_map <- map_data("world") %>% 
  filter(region == "China" | region == "Russia") %>% 
