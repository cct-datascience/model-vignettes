...
* BioCro config file (copied from a PEcAn run by generate_weather_config.R and updated by collate_biomass.R)
...
opt_weather <- read.csv("~/model-vignettes/BioCro/DARPA/biomass_opti/inputs/ch_weather.csv")
colnames(opt_weather) <- c("year", "doy", "hour", "Solar", "Temp", "RH", "WS", "precip")

config <- PEcAn.BIOCRO::read.biocro.config("~/model-vignettes/BioCro/DARPA/biomass_opti/inputs/ch_config.xml")
...
opt_biomass <- read.csv("~/model-vignettes/BioCro/DARPA/biomass_opti/inputs/ch_biomass.csv")
```

We want biomass estimates from `BioGro` (i.e., ttt) to be as close to the actual biomass values as possible (i.e., opt_biomass). Will be optimizing over the parameters, which are the biomass coefficients. 

To allow grain to accumulate prior to the final timestep, create an updated phenoParms function. 
```{r}
library(BioCro)
phenoParms <- function(tp1=562, tp2=1312, tp3=2063, tp4=2676, tp5=3211, tp6=7000,
                       ...
                       kGrain1 = 0, kGrain2 = 0, kGrain3 = 0, kGrain4 = 0, kGrain5=0, kGrain6=0){
  list(tp1=tp1, tp2=tp2, tp3=tp3, tp4=tp4, tp5=tp5, tp6=tp6,
       ...
       kStem6=kStem6, kLeaf6=kLeaf6, kRoot6=kRoot6, kRhizome6=kRhizome6, 
       kGrain1=kGrain1, kGrain2=kGrain2, kGrain3=kGrain3, kGrain4=kGrain4, kGrain5=kGrain5, kGrain6=kGrain6)
}
valid_dbp <- function(x, tol = 1) return(x)
```
...
opfn <- function(optimizingParms, thermaltimeParms, rhizomeParms){ 
  optimizingParms[1:4] <- optimizingParms[1:4]/sum(optimizingParms[1:4])
  optimizingParms[5:8] <- optimizingParms[5:8]/sum(optimizingParms[5:8])
  optimizingParms[9:12] <- optimizingParms[9:12]/sum(optimizingParms[9:12])
  optimizingParms[13:16] <- optimizingParms[13:16]/sum(optimizingParms[13:16])
  optimizingParms[17:20] <- optimizingParms[17:20]/sum(optimizingParms[17:20])
  optimizingParms[21:24] <- optimizingParms[21:24]/sum(optimizingParms[21:24])
  params_list <- phenoParms(tp1 = thermaltimeParms[1], 
                           ...
                           kGrain1 = optimizingParms[4],
                           kStem2 = optimizingParms[5], 
                           kLeaf2 = optimizingParms[6],
                           kRoot2 = optimizingParms[7],
                           kGrain2 = optimizingParms[8],
                           kStem3 = optimizingParms[9], 
                           kLeaf3 = optimizingParms[10],
                           kRoot3 = optimizingParms[11],
                           kGrain3 = optimizingParms[12],
                           kStem4 = optimizingParms[13], 
                           kLeaf4 = optimizingParms[14],
                           kRoot4 = optimizingParms[15],
                           kGrain4 = optimizingParms[16],
                           kStem5 = optimizingParms[17], 
                           kLeaf5 = optimizingParms[18],
                           kRoot5 = optimizingParms[19],
                           kGrain5 = optimizingParms[20],
                           kStem6 = optimizingParms[21], 
                           kLeaf6 = optimizingParms[22],
                           kRoot6 = optimizingParms[23],
                           kGrain6 = optimizingParms[24],
...
optimizingParms_check <- rep(c(0.2, 0.3, 0.4, 0.1), 6)
