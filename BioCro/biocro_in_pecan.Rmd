...
3. The first needed file is an XML that contains the configuration information required to get the data and run the model. This example run is for the Setaria wild type PFT for a site at the Donald Danforth Center in St. Louis, MO using weather data from the entire year of 2004. We are also running ensemble analyse and a meta-analysis, but no sensitivity analysis. 
...
6. The results for these files will appear in a new folder called `biocro_darpa_results`. 
    ...
    To test if this works, plot the output data for leaf area index across time. In the console or a new R script, type the following R command and the plot shown should be generated. 
...
```{r, echo=FALSE, out.width="400px"}
knitr::include_graphics("biocro_lai_welsch_setaria.png")
```